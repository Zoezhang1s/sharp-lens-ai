...
- 240–380 words total.`;
    ...
    const identityLock = language === "zh"
      ? "【绝对身份锁】：必须是原图同一个人，同一张脸，同一五官比例，同一眼型鼻型嘴型眉形，同一发型发色，同一肤色，同一年龄感；必须穿着原图同一身衣服与配饰（款式、颜色、材质、花纹、鞋包首饰眼镜全部一致），不允许换人、换脸、改服装、AI重绘成陌生长相。【人脸锐度锁】：人脸必须高清、干净、锐利，对焦点必须落在眼睛上，眼睛鼻子嘴巴结构完整自然，绝不能模糊、拉扯、扭曲、双眼不对称、糊五官、塑料皮、嘴部错位、牙齿融合、手指畸形。【重新策划要求】：场景、背景、地点、构图、光线、姿势、动作、表情、机位、角度、焦段、色调全部都要重新设计，目标是为这个人和这身衣服打造一张真正高级、有亮点、可以当作参考的人像作品；成片在场景、姿势、构图、光线上必须与原图明显不同，不能只是小修小补。"
      : "[ABSOLUTE IDENTITY LOCK]: must be the exact same person from the original — same face, same feature proportions, same eyes, nose, mouth, brows, same hair, same skin tone, same age impression; must wear the exact same outfit and accessories from the original (style, color, material, pattern, shoes, bag, jewelry, glasses all identical). No face swap, no new person, no outfit change, no AI replacement face. [FACE SHARPNESS LOCK]: face must be clean, high-resolution, and sharp with eyes in focus; eyes, nose, mouth, skin texture, teeth, and fingers must remain natural and undistorted. No blur, stretch, asymmetry, mushy features, plastic skin, warped mouth, fused teeth, or deformed hands. [RE-PLAN REQUIREMENT]: scene, background, location, composition, lighting, pose, action, expression, camera angle, focal length, color tone must all be redesigned — the goal is to deliver a genuinely high-end, look-worthy reference portrait of this person in this outfit. The result must clearly differ from the original in scene, pose, composition, and lighting; it must not be a light tweak.";
    const negativeLock = language === "zh"
      ? "负面约束：禁止换脸、禁止陌生人、禁止韩式脸/AI网红脸、禁止改变五官比例、禁止改发型、禁止改服装款式颜色花纹、禁止改年龄感和身材、**绝对禁止任何摄影器材或拍摄现场穿帮入画**（补光灯/柔光箱/灯架/反光板/影棚背景纸/三脚架/相机/麦克风/稳定器/收音杆/伞灯/闪光灯灯头/电线/剧组人员/化妆师/助理/路人围观一律不许出现）、**只能输出最终成片，禁止幕后照/拍摄花絮/工作照/任何中间状态**、禁止千篇一律奶油暖调、禁止柔焦糊脸、禁止脸部和手部畸形、禁止只有磨皮不修问题、禁止输出与原图场景姿势构图都几乎相同的结果。"
      : "Negative: no face swap, no different person, no influencer AI face, no altered facial proportions, no changed hair, no changed outfit style/color/pattern, no changed age/body impression. **ABSOLUTELY NO photo equipment or behind-the-scenes leaks in frame** (no softboxes, fill lights, light stands, reflectors, studio backdrops, tripods, cameras, microphones, gimbals, boom poles, umbrella lights, strobes, cables, crew, makeup artists, assistants, onlookers). **Output ONLY the finished final photo — never a BTS / making-of / work-in-progress shot.** No generic creamy warm grading, no soft blurry face, no face/hand deformation, no simple beauty retouch, and no output whose scene/pose/composition still resembles the original.";
    ...
    const imageEditInstruction = language === "zh"
      ? `${imagePrompt}\n\n额外强约束：把原图作为人脸与服装的身份参考，但要为这个人和这身衣服重新策划并拍摄一张完全不同、明显更高级、有亮点的参考照片。场景、背景、构图、光线、姿势、动作、表情、机位都必须重新设计，**不能只是在原图基础上微调**。人脸必须清晰锐利、五官稳定自然；服装必须和原图完全一致；成片必须是一张真正好看的人像作品。输出一张完成图，不要解释。`
      : `${imagePrompt}\n\nAdditional hard constraint: treat the original photo as a reference for face identity and outfit only. Re-plan and re-shoot a completely different, clearly more elevated, look-worthy reference portrait of this person in this outfit. Scene, background, composition, lighting, pose, action, expression, and camera angle must all be redesigned — **do NOT lightly tweak the original**. Keep the face sharp and natural; keep the outfit identical to the original; deliver a genuinely beautiful portrait. Output one finished image only, no explanation.`;
