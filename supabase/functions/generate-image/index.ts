...
- 240–380 words total.`;
    ...
    const identityLock = language === "zh"
      ? `【绝对身份锁】：必须是原图同一个人，同一张脸，同一五官比例，同一眼型鼻型嘴型眉形，同一发型发色，同一肤色，同一年龄感；必须穿着原图同一身衣服与配饰（款式、颜色、材质、花纹、鞋包首饰眼镜全部一致），不允许换人、换脸、改服装、AI重绘成陌生长相。【人脸锐度锁】：人脸必须高清、干净、锐利，对焦点必须落在眼睛上，眼睛鼻子嘴巴结构完整自然，绝不能模糊、拉扯、扭曲、双眼不对称、糊五官、塑料皮、嘴部错位、牙齿融合、手指畸形。【场景主题延续锁】：必须保留原图的场景主题（同一类型的地点：海边/街头/咖啡馆/室内/天台/公园/校园/商场/家中等），严禁跳到完全无关的另一个地点，但必须在同主题下换到更干净、更有氛围、更出片的角落、机位、时间段，把原场景做出升级版。
【强制大幅改变清单 — 以下7项必须每一项都和原图明显不同，否则视为失败】：
① 机位高度必须改变（如原图平视 → 必须改成低角度仰拍、或高角度俯拍、或极低地面视角、或过肩视角）；
② 景别/焦段必须改变（如原图全身/中景 → 改成半身、特写、远景、或反过来；焦段切换35mm广角/50mm/85mm/135mm长焦）；
③ 人物身体朝向必须改变（如原图正面对镜头 → 改成侧身、背身回眸、3/4侧面、全侧、低头、抬头等全新朝向）；
④ 姿势动作必须完全重新设计（绝对不允许复制原图姿势，要从行走、转身、坐下、蹲下、靠墙、伸展、低头玩头发、与道具互动、与环境互动等全新动作里选一个明显不同的）；
⑤ 主光方向和时间段必须改变（如原图平淡散射光 → 改成戏剧性侧逆光/纯逆光/顶光/低角度光/窗光/霓虹光，时间段也要切换：黄金时刻/蓝调时刻/夜景/清晨等）；
⑥ 背景元素必须改变（在同主题下换一个完全不同的角落、不同的前景遮挡、不同的纵深层次，原图背景里的具体物体绝不能再出现在画面同样的位置；要找一个比原图更上镜的机位）；
⑦ 色调氛围必须改变（如原图灰平淡 → 改成电影感冷暖对比、或胶片复古、或高对比暗调、或高调通透、或浓郁油画感）。
【最终成片自检】：把新图和原图放一起对比，必须一眼能看出"这是同一个人同一身衣服在同类型场景里，但是完全两张不同的作品"，绝不允许"看起来像同一组连拍的两张"。`
...
[MANDATORY DRASTIC-CHANGE CHECKLIST — every one of these 7 must clearly differ from the original, or it's a failure]:
① Camera height MUST change (e.g. eye-level → low-angle upward / high-angle downward / extreme ground level / over-the-shoulder);
② Framing & focal length MUST change (full-body/medium → close-up, half-body, or wide; switch focal length: 35mm wide / 50mm / 85mm / 135mm tele);
③ Body orientation MUST change (front-facing → side profile / back-turned glance / 3/4 / full side / head down / head up);
④ Pose and action MUST be completely redesigned (NEVER copy the original pose — pick a visibly different new action: walking, turning, sitting, crouching, leaning, stretching, hair-touching, prop interaction, environment interaction, etc.);
⑤ Key-light direction AND time of day MUST change (flat diffuse → dramatic side-back / full backlight / top / low-angle / window / neon; switch to golden hour / blue hour / night / early morning);
⑥ Background elements MUST change (within the same theme, switch to a completely different corner with different foreground occlusion and depth layering — specific objects from the original background must NEVER appear in the same position; pick a more photogenic angle than the original);
⑦ Color tone and mood MUST change (flat gray → cinematic warm-cool contrast / retro film / high-contrast moody / bright airy / rich oil-painting feel).
[FINAL SELF-CHECK]: when placed side-by-side with the original, it must be instantly obvious that "this is the same person in the same outfit and same scene type, but two completely different photographs" — NEVER look like "two frames from the same burst".`;
    const negativeLock = language === "zh"
      ? "负面约束：禁止换脸、禁止陌生人、禁止韩式脸/AI网红脸、禁止改变五官比例、禁止改发型、禁止改服装款式颜色花纹、禁止改年龄感和身材、禁止跳到与原图主题完全无关的另一个地点、**绝对禁止任何摄影器材或拍摄现场穿帮入画**（补光灯/柔光箱/灯架/反光板/影棚背景纸/三脚架/相机/麦克风/稳定器/收音杆/伞灯/闪光灯灯头/电线/剧组人员/化妆师/助理/路人围观一律不许出现）、**只能输出最终成片，禁止幕后照/拍摄花絮/工作照/任何中间状态**、禁止千篇一律奶油暖调、禁止柔焦糊脸、禁止脸部和手部畸形、禁止只有磨皮不修问题、**严禁与原图在机位高度/景别焦段/身体朝向/姿势动作/主光方向/背景元素/色调中任何一项保持相同**——任何一项与原图相同都算彻底失败。"
      : "Negative: no face swap, no different person, no influencer AI face, no altered facial proportions, no changed hair, no changed outfit style/color/pattern, no changed age/body impression, no jumping to a location whose theme is unrelated to the original. **ABSOLUTELY NO photo equipment or behind-the-scenes leaks in frame** (no softboxes, fill lights, light stands, reflectors, studio backdrops, tripods, cameras, microphones, gimbals, boom poles, umbrella lights, strobes, cables, crew, makeup artists, assistants, onlookers). **Output ONLY the finished final photo — never a BTS / making-of / work-in-progress shot.** No generic creamy warm grading, no soft blurry face, no face/hand deformation, no simple beauty retouch. **STRICTLY FORBIDDEN to keep ANY of camera-height / framing / focal-length / body-orientation / pose / key-light / background / color-tone identical to the original** — keeping any one identical counts as total failure.";
    ...
    const imageEditInstruction = language === "zh"
      ? `${imagePrompt}\n\n【最重要的额外强约束】：把原图仅作为人脸身份、服装款式、场景主题这三项的参考。新图必须满足以下硬性条件，否则失败：\n(a) 机位高度、景别焦段、人物身体朝向、姿势动作、主光方向、背景具体元素、整体色调 —— 这7项必须每一项都和原图明显不同；\n(b) 场景类型必须保持一致（同样是海边/街头/室内/咖啡馆/森林/天台等），但要换到同主题下完全不同的角落、不同的机位、不同的时间段；\n(c) 人脸必须清晰锐利，五官稳定自然，对焦在眼睛；\n(d) 服装与原图完全一致；\n(e) 必须是一张有摄影感、有光影、有氛围的高级人像作品，不是修图，不是滤镜，不是原图微调。\n直接输出一张完成图，不要解释。`
      : `${imagePrompt}\n\n[CRITICAL EXTRA CONSTRAINT]: treat the original photo ONLY as a reference for three things — face identity, outfit, and scene theme. The new image MUST satisfy these hard requirements, or it fails:\n(a) Camera height, framing/focal length, body orientation, pose & action, key-light direction, specific background elements, overall color tone — ALL 7 must clearly differ from the original;\n(b) Scene type must stay the same (still beach / street / indoor / cafe / forest / rooftop etc.), but switch to a completely different corner, angle, and time of day within that same theme;\n(c) Face must be sharp, features natural, eyes in focus;\n(d) Outfit must be identical to the original;\n(e) Must be a high-end portrait with intentional light, mood, and photographic quality — not a retouch, filter, or light tweak of the original.\nOutput one finished image only, no explanation.`;
